...
const AVATAR_COLORS = [
  ...
  'bg-amber-600','bg-rose-600','bg-blue-600','bg-pink-600',
]
...
function Avatar({ name = '', size = 'sm' }) {
  ...
  return (
    <div className={`${sz} ${color} rounded-full flex items-center justify-center font-bold text-white shrink-0 select-none`}>
      {initials}
    </div>
  )
}
...
function Badge({ status }) {
  return (
    <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLES[status] || STATUS_STYLES.New}`}>
      {status || 'New'}
    </span>
  )
}
...
function Tag({ label, onRemove }) {
  return (
    <span className="inline-flex items-center gap-1 text-[11px] font-medium bg-blue-500/10 border border-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full">
      {label}
      {onRemove && (
        <button onClick={() => onRemove(label)} className="text-blue-300 hover:text-red-400 transition-colors leading-none">&times;</button>
      )}
    </span>
  )
}
...
function ContactModal({ contact, agents, onSave, onClose }) {
  ...
  const [form, setForm] = useState({
    wa_id: contact?.wa_id || '', profile_name: contact?.profile_name || '',
    email: contact?.email || '', tags: contact?.tags || [],
    status: contact?.status || 'New', assigned_to: contact?.assigned_to || '',
    notes: contact?.notes || '', opted_in: contact?.opted_in || false,
  })
  ...
  const validate = () => {
    ...
    else if (!/^\d{7,15}$/.test(clean)) e.wa_id = 'Enter digits only with country code (e.g. 919876543210)'
    setErrors(e)
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-6 pb-0">
          <h2 className="text-base font-semibold">{isEdit ? 'Edit Contact' : 'Add Contact'}</h2>
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          {errors.submit && (
            <div className="bg-red-900/20 border border-red-800/40 text-red-400 text-sm px-4 py-2.5 rounded-xl">{errors.submit}</div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                disabled={isEdit}
                className={`w-full bg-slate-800 border rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 outline-none transition-colors font-mono
                  ${errors.wa_id ? 'border-red-500 focus:border-red-400' : 'border-slate-700 focus:border-blue-500'}`}
              />
              {errors.wa_id && <p className="text-xs text-red-400 mt-1">{errors.wa_id}</p>}
              <p className="text-xs text-slate-500 mt-1">Country code + number, no + sign</p>
            </div>

            {/* Name */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1.5">Full name</label>
              <input value={form.profile_name} onChange={set('profile_name')} placeholder="John Doe"
            ...
            {/* Agent */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1.5">Assign agent</label>
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 space-y-2">
                {form.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {form.tags.map(t => <Tag key={t} label={t} onRemove={t => setForm(p => ({ ...p, tags: p.tags.filter(x => x !== t) }))} />)}
                  </div>
                )}
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={addTag}
                    className="text-xs text-blue-400 hover:text-blue-300 font-medium transition-colors px-1">Add</button>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-2">
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={form.opted_in} onChange={e => setForm(p => ({ ...p, opted_in: e.target.checked }))}
                  className="w-4 h-4 rounded accent-emerald-500" />
                <span className="text-sm text-slate-300">Opted in for WhatsApp messages</span>
              </label>
            </div>
          </div>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold">{contact.profile_name || <span className="text-slate-400">No name</span>}</p>
              <p className="text-xs text-slate-400 font-mono mt-0.5">+{contact.wa_id}</p>
              <Badge status={contact.status} />
            </div>
          </div>

          {/* Open chat */}
          <button onClick={() => onOpenChat(contact)}
            ...
            <svg viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5"><path d="M2 5a2 2 0 012-2h12a2 2 0 012 2v8a2 2 0 01-2 2H6l-4 4V5z"/></svg>
            Open Chat
          </button>

          {/* Info */}
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-3">Info</p>
            <div className="space-y-2">
              {[
                { label: 'Phone',    value: `+${contact.wa_id}`, mono: true },
                { label: 'Email',    value: contact.email || '—' },
                { label: 'Opted in', value: contact.opted_in ? '✓ Yes' : '—', cls: contact.opted_in ? 'text-emerald-400' : 'text-slate-500' },
                { label: 'Added',    value: fmt(contact.created_at) },
                { label: 'Agent',    value: agents.find(a => a.id === contact.assigned_to)?.name || '—' },
              ].map(r => (
                <div key={r.label} className="flex justify-between items-center py-1.5 border-b border-slate-800/50 last:border-0">
                  <span className="text-xs text-slate-500">{r.label}</span>
                  <span className={`text-xs text-right max-w-[160px] truncate ${r.cls || 'text-slate-300'} ${r.mono ? 'font-mono' : ''}`}>{r.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {(contact.tags || []).length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {contact.tags.map(t => <Tag key={t} label={t} />)}
              </div>
            </div>
          )}
          ...
          {contact.notes && (
            <div>
              ...
              <p className="text-xs text-slate-300 bg-slate-800 rounded-xl px-3 py-2.5 leading-relaxed">{contact.notes}</p>
            </div>
          )}
        </div>
...
function CSVModal({ onClose, onDone }) {
  ...
  const handleFile = e => {
    ...
    reader.onload = ev => {
      const data = parseCSV(ev.target.result); setCsv(data)
      const auto = {}
      FIELDS.forEach(f => {
        const m = data.headers.find(h =>
          h === f.key || (f.key === 'wa_id' && (h.includes('phone') || h.includes('mobile'))) ||
  ...
  const runImport = async () => {
    setLoading(true)
    const contacts = csv.rows.map(r => ({
      wa_id:        (r[mapping.wa_id] || '').replace(/[\s+\-()]/g, ''),
      profile_name: r[mapping.profile_name] || '',
      email:        r[mapping.email] || '',
      tags:         r[mapping.tags] ? r[mapping.tags].split(';').map(t => t.trim()).filter(Boolean) : [],
      status:       r[mapping.status] || 'New',
    })).filter(c => c.wa_id && /^\d{7,15}$/.test(c.wa_id))
    try {
      const { data } = await api.post('/contacts/bulk', { contacts })
      setResult(data); setStep('result'); onDone()
    } catch (e) {
      setResult({ error: e.response?.data?.detail || 'Import failed' }); setStep('result')
    }
    setLoading(false)
  }
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex px-5 pt-4 gap-1">
          {['upload','map','preview','result'].map((s, i) => (
            <div key={s} className={`h-1 flex-1 rounded-full transition-colors ${
              ['upload','map','preview','result'].indexOf(step) >= i ? 'bg-blue-500' : 'bg-slate-700'}`} />
          ))}
        </div>
        ...
        {step === 'upload' && (
          <div className="p-6">
            ...
            >
              ...
              <p className="text-sm font-medium text-slate-300 group-hover:text-white transition-colors">Click to upload CSV</p>
              <p className="text-xs text-slate-500 text-center leading-relaxed">
                Must have a phone column with country code (no +).<br/>Tags: semicolon-separated.
              </p>
              <button type="button" className="mt-2 px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-medium rounded-xl transition-colors">
                Choose file
              </button>
            </div>
          </div>
        ...
        {step === 'map' && csv && (
          <div className="p-6 space-y-4">
            <p className="text-sm text-slate-400">Found <span className="font-semibold text-white">{csv.rows.length}</span> rows. Map columns:</p>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setStep('upload')} className="px-4 py-2 text-xs text-slate-400 hover:text-slate-200 bg-slate-800 border border-slate-700 rounded-xl transition-colors">Back</button>
              <button onClick={() => {
                setPreview(csv.rows.slice(0, 5).map(r => ({
                  wa_id: (r[mapping.wa_id]||'').replace(/[\s+\-()]/g,''),
                  profile_name: r[mapping.profile_name]||'', email: r[mapping.email]||'',
                  tags: r[mapping.tags]?r[mapping.tags].split(';').map(t=>t.trim()):[], status: r[mapping.status]||'New',
                }))); setStep('preview')
        ...
        {step === 'preview' && (
          <div className="p-6 space-y-4">
            ...
            <div className="overflow-x-auto border border-slate-800 rounded-xl">
              <table className="w-full text-xs">
                <thead className="bg-slate-800/60">
                  <tr>{['Phone','Name','Tags','Status'].map(h => <th key={h} className="px-3 py-2.5 text-left text-slate-400 font-semibold">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {preview.map((r, i) => (
                    <tr key={i} className="border-t border-slate-800/60">
                      <td className="px-3 py-2 font-mono text-slate-300">{r.wa_id || <span className="text-red-400">missing</span>}</td>
                      <td className="px-3 py-2 text-slate-300">{r.profile_name || '—'}</td>
                      <td className="px-3 py-2"><div className="flex flex-wrap gap-1">{r.tags.map(t => <Tag key={t} label={t} />)}</div></td>
            ...
            <p className="text-xs text-slate-500">Will import all {csv.rows.length} rows (invalid phones skipped).</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setStep('map')} className="px-4 py-2 text-xs text-slate-400 hover:text-slate-200 bg-slate-800 border border-slate-700 rounded-xl transition-colors">Back</button>
            ...
            {result.error ? (
              ...
              <>
                ...
                <div className="flex gap-8 mt-2">
                  {[['Created', result.created, 'text-emerald-400'], ['Skipped', result.skipped, 'text-slate-400'], ['Total', result.total, 'text-white']].map(([l, v, c]) => (
...
export default function Contacts() {
  ...
  const load = useCallback(async (p = 1) => {
    ...
    try {
      ...
      setContacts(data.contacts || []); setTotal(data.total || 0); setPage(p)
    } catch {}
    setLoading(false)
  ...
  const deleteSelected = async () => {
    ...
    try { await Promise.all([...selected].map(id => api.delete(`/contacts/${id}`))); setSelected(new Set()); load(1) }
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...contacts.map(c => [c.profile_name||'', c.wa_id, c.email||'', (c.tags||[]).join(';'), c.status||'New', c.opted_in?'Yes':'No', fmt(c.created_at)])
    ]
  ...
  const openChat = c => navigate('/dashboard/inbox', { state: { wa_id: c.wa_id, contact_id: c.id, contact_name: c.profile_name } })
    ...
    <div className="p-6 max-w-screen-xl font-sans text-slate-100">
      ...
      <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportCSV}
            ...
            <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5"><path d="M2 13h12v1.5H2zm6-1.5L4 7h3V2h2v5h3z"/></svg>
            Export
          </button>
          <button onClick={() => setShowImport(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 rounded-xl transition-colors">
            <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5"><path d="M2 13h12v1.5H2zm6-9.5L12 8H9v5H7V8H4z"/></svg>
            Import CSV
          </button>
      ...
      <div className="flex flex-wrap items-center gap-3 p-4 bg-slate-900/60 border border-slate-800 rounded-2xl mb-4">
        <div className="relative flex-1 min-w-48">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-500" viewBox="0 0 20 20" fill="none">
        ...
        {[
          { value: filterStatus, set: setFilterStatus, opts: STATUSES, label: 'All statuses' },
          { value: filterTag,    set: setFilterTag,    opts: allTags,  label: 'All tags'    },
          { value: filterAgent,  set: setFilterAgent,  opts: agents.map(a => ({ v: a.id, l: a.name })), label: 'All agents' },
        ].map((f, i) => (
        ...
        {(search || filterStatus || filterTag || filterAgent) && (
          <button
            onClick={() => { setSearch(''); setFilterStatus(''); setFilterTag(''); setFilterAgent('') }}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-800/40">
                ...
                {['Contact','Phone','Tags','Status','Opted In','Agent','Added',''].map(h => (
            ...
            <tbody>
                ...
                <tr key={i} className="border-b border-slate-800/60">
                  {[...Array(9)].map((_, j) => (
                    <td key={j} className="px-4 py-3">
                      <div className="h-4 bg-slate-800 rounded animate-pulse" style={{ width: ['32px','140px','100px','120px','60px','80px','80px','80px','60px'][j] }} />
                    </td>
                  ))}
                </tr>
              ))}
                ...
                <tr>
                  <td colSpan={9}>
                    <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-500">
                      ...
                      <p className="text-sm font-medium text-slate-400">No contacts found</p>
                      <button onClick={() => setShowAdd(true)}
              ...
              {!loading && contacts.map(c => (
                ...
                >
                  <td className="px-4 py-3" onClick={e => { e.stopPropagation(); toggleSel(c.id) }}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-200 leading-tight">
                          {c.profile_name || <span className="text-slate-500 italic text-xs">No name</span>}
                        </p>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs font-mono text-slate-400">+{c.wa_id}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1 max-w-[160px]">
                      {(c.tags || []).slice(0, 2).map(t => <Tag key={t} label={t} />)}
                      {(c.tags || []).length > 2 && (
                        <span className="text-[11px] text-slate-500 bg-slate-800 px-1.5 py-0.5 rounded-full">+{c.tags.length - 2}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3"><Badge status={c.status || 'New'} /></td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium ${c.opted_in ? 'text-emerald-400' : 'text-slate-600'}`}>
                      {c.opted_in ? '✓' : '—'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs text-slate-500">{agents.find(a => a.id === c.assigned_to)?.name || '—'}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs text-slate-500 whitespace-nowrap">{fmt(c.created_at)}</span>
                  </td>
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openChat(c)}
                        className="p-1.5 text-slate-500 hover:text-emerald-400 hover:bg-emerald-900/20 rounded-lg transition-colors" title="Open chat">
                        <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5"><path d="M1 4a2 2 0 012-2h10a2 2 0 012 2v6a2 2 0 01-2 2H5L1 15V4z"/></svg>
                      </button>
                      <button onClick={() => setEditContact(c)}
                        className="p-1.5 text-slate-500 hover:text-blue-400 hover:bg-blue-900/20 rounded-lg transition-colors" title="Edit">
                        <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5"><path d="M11.5 2a1.5 1.5 0 012.12 2.12l-8 8-2.83.71.71-2.83 8-8z"/></svg>
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      {(showAdd || editContact) && (
        <ContactModal
          contact={editContact}
          agents={agents}
          onSave={() => { setEditContact(null); setShowAdd(false); load(page) }}
          onClose={() => { setEditContact(null); setShowAdd(false) }}
        />
      )}
      ...
      {drawerContact && (
        <ContactDrawer
          ...
          agents={agents}
          onUpdate={u => { setContacts(p => p.map(c => c.id===u.id?u:c)); setDrawerContact(u) }}
